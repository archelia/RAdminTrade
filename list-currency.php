...
<div id="cari" style="margin-top:1.5em;">

</div>

<div id="list">
<table width="50%" cellspacing="2" cellpadding="2" align="center" bgcolor="#A9D0F5" border="0">
<tr>
	<td align="left" bgcolor="#FFFFFF" colspan="2"><h4>Currency List</h4></td>
</tr>
<tr>
	<form name="formcari" id="formcari" method="POST" action="list-currency.php">
...
<tr>
	...
	<tr>
		...
		<td align="center" width="">Currency Name</td>
		<td align="center" width="10%">Symbol</td>
	...
	<?php
	$query = "SELECT c.*, t.countryname FROM currency c, country t ";
	$query .= "WHERE c.idcountry=t.idcountry ";

	if (isset($_POST['search']))
	{
		$tekscari=$_POST['tekscari'];
		$query .="AND (currname LIKE '$tekscari%' OR symbol LIKE '$tekscari%') ";
	}
	$query .="ORDER BY currname ASC ";
	$query1 = $query;
	...
	{
		echo '<tr>
			<td align="left">'.$row["idcurr"].'</td>
			<td align="left">'.$row["currname"].'</td>
			<td align="center">'.$row["symbol"].'</td>
			<td align="left">'.$row["countryname"].'</td>';	
		echo'<td align="center">';
			?>
			<form action="edit-currency.php" method="POST" onsubmit="return confirm('Are you sure want to edit this data?');">
			<?php
			echo '	<input type="submit" name="submit" id="submit" value="Edit">
				<input type="hidden" name="ide" id="ide" value='.$row["idcurr"].'>
				</form> ';
		echo '</td>
			<td align="center">';
			...
			echo '	<input type="submit" name="submit" id="submit" value="Delete">
				<input type="hidden" name="ide" id="ide" value='.$row["idcurr"].'>
				<input type="hidden" name="tabel" id="tabel" value="currency">		
				<input type="hidden" name="page" id="page" value="list-currency.php">
				<input type="hidden" name="namaid" id="namaid" value="idcurr">	
				</form> ';}
		echo '</td>
			</tr> ';
	}
...
<tr>
	...
	<form action="create-currency.php" method="POST">
	<td width="60%" align="left">
		<input type="submit" name="createnew" id="createnew" value="Click here to create new currency">
	</td>
	</form>	
</tr>
</table>
