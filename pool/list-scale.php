...
<tr>
	<td align="left" bgcolor="#FFFFFF" colspan="2"><h4>Scale List</h4></td>
</tr>
<tr>
	
	<form name="formcari" id="formcari" method="POST" action="list-Scale.php">
...
<tr>
	...
	<tr>
		...
		<td align="center" width="">Scale</td>
	...
	$query1 = $query."ORDER BY scale asc ";

	if (isset($_POST['search']))
	{
		$tekscari=$_POST['tekscari'];
		$query .="WHERE scale LIKE '$tekscari%' ORDER BY idscale ASC LIMIT $posisi,$batas";
	}
	else
	$query .="ORDER BY idscale ASC LIMIT $posisi,$batas";
	$result = mysql_query($query) or die ("Ada yang salah !");
	$result=mysql_query($query);
	while($row = mysql_fetch_array($result))
	{
		echo '<tr>
			<td align="left">'.$row["idscale"].'</td>
			<td align="left">'.$row["scale"].'</td>
			<td align="center">';
			?>
			<form action="edit-scale.php" method="POST" onsubmit="return confirm('Are you sure want to edit this data?');">
			<?php
			echo '	<input type="submit" name="submit" id="submit" value="Edit">
				<input type="hidden" name="ide" id="ide" value='.$row["idscale"].'>
				</form> ';
			echo '</td>
			<td align="center">';
			...
			echo '	<input type="submit" name="submit" id="submit" value="Delete">
				<input type="hidden" name="ide" id="ide" value='.$row["idscale"].'>
				<input type="hidden" name="tabel" id="tabel" value="scale">		
				<input type="hidden" name="page" id="page" value="list-Scale.php">
				<input type="hidden" name="namaid" id="namaid" value="idscale">	
				</form> ';
		echo '</td>
			</tr> ';
	}
...
<tr>
	...
	<form action="create-Scale.php" method="POST">
	<td width="60%" align="left">
		<input type="submit" name="createnew" id="createnew" value="Click here to create new Scale">
	</td>
	</form>	
</tr>
</table>
