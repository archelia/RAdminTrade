...
<table width="85%" cellspacing="2" cellpadding="2" align="center" bgcolor="#A9D0F5" border="0">
<tr>
	<td align="left" bgcolor="#FFFFFF" colspan="2"><h4>Product List</h4></td>
</tr>
<tr>
	<form name="formcari" id="formcari" method="GET" action="list-products.php">
	...
	<td align="right">Order By :
		<select name="urutan" id="urutan" onchange="document.getElementById('formcari').submit();">
			<option value="1" selected>ID Product Ascending</option>
			<option value="2">ID Product Descending</option>
			<option value="3">Product Name A-Z</option>
			<option value="4">Product Name Z-A</option>
			<option value="5">Input Date Newest</option>
			<option value="6">Input Date Longest</option>
		</select>	
	</td>
	</form>	
</tr>
...
<tr>
	...
	<tr>
		...
		<td align="center" width="14%">Subcategory</td>
		<td align="center" width="7%">Status</td>
		<td align="center" width="8%">Detail</td>
	...
	if (isset($_GET['tekscari']))
	{
		if ($_GET['pilihancari']!="kosong")
		{
		$tekscari=$_GET['tekscari'];
		$pilihan=$_GET['pilihancari'];
	...
	{
		$urutan=$_GET['urutan'];
		if ($urutan=='1'){$urutan='idproduct asc';}
		else if ($urutan=='2'){$urutan='idproduct desc';}
		else if ($urutan=='3'){$urutan='productname asc';}
		else if ($urutan=='4'){$urutan='productname desc';}
		else if ($urutan=='5'){$urutan='inputdate desc';}
		else if ($urutan=='6'){$urutan='inputdate asc';}
		$query .=" ORDER BY $urutan ";
	}
	else $query .="ORDER BY p.productname asc ";
	$query1=$query;
	...
	while($row = mysql_fetch_array($result))
	{
		...
		echo '<td align="left">'.$row["idproduct"].'</td>
			<td align="left"><b>'.$row["productname"].'</b><br><br><br><span class="keterangan" style="margin-top:5px;">';
				echo "&nbsp;&#187;&nbsp;added ";
				$nowtime = time();
				//echo $row["inputdate"];
		...
		if ($row["active"]==0){echo "Deleted";} else {
			...
			echo '<input type="submit" name="submit" id="submit" value="Delete">
				<input type="hidden" name="ide" id="ide" value='.$row["idproduct"].'>
				<input type="hidden" name="tabel" id="tabel" value="product">
...
<tr>
	<td colspan="2" align="center">
		...
		if (isset($_GET['tekscari']))
		{
			$file.="tekscari=".$_GET['tekscari']."&";
			$file.="pilihancari=".$_GET['pilihancari']."&";
		}
		if (isset($_GET['urutan']))
		{$file.="urutan=".$_GET['urutan'];}
